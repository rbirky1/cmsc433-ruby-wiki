...
class Page
	attr_accessor :topic, :in_degree, :out_degree, :depth
...
# depth+=1
# # Keep track of depth
# elementsToDepthIncrease = $branching
# nextToDepthIncrease = 0

# Comparison array
visited_array = [topic.to_sym]

# one = visited_objects[0]
# puts "#{one} (#{one.in_degree}/#{one.out_degree})"
...
while !to_visit.empty? do
	...
	if visited_array.include?(this_topic.to_sym)
		...
		this_to_visit = this_links[0...$branching]

		# get in and outdegree
		this_in_degree = this_links.length
		this_out_degree = get_out_degree($base_url, this_topic)

		visited_objects.push(Page.new(this_topic,this_in_degree,this_out_degree,depth,this_to_visit))

		# puts "CURRENT"
		# puts this_to_visit
		# puts "*******"
		# # add new links, check depth here
		# if (depth < $distance)
		# 	to_visit += this_to_visit
		# end

		# nextToDepthIncrease += ($branching**depth)
		# elementsToDepthIncrease -= 1
		# if (elementsToDepthIncrease == 0)
		# 	elementsToDepthIncrease = nextToDepthIncrease
		# 	nextToDepthIncrease = 0
		# 	depth += 1
		# end
